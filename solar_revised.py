...
def get_observer_location():
    """ Returns the longitude and latitude for the geolocation of this machine, based on its IP address.
        NOTE:  IP address is entered manually below; autodetected IP is the Amazon Workspace machine
        in a data center, probably in Ashburn VA."""
...
def get_observer_elevation(loc_lat, loc_lon):
    ...
    response = requests.get(f"https://api.open-elevation.com/api/v1/lookup?locations={loc_lat},{loc_lon}")
    data = response.json()
    loc_elev = data["results"][0]["elevation"]
    return loc_elev

get_observer_elevation()


def get_body_position(loc_lat, loc_lon, elevation, astro_body, date_entered, time_entered):
    ...
    response = requests.get(f"https://api.astronomyapi.com/api/v2/bodies/positions/{astro_body}/",
        auth=(constant1, constant2),params=params,)
    data = response.json()
